...
(function () {
  ...
  async function init(container) {
      ...
      <div id="map-wrap" style="position:relative;width:100%;height:100%;">
        ...
        <div class="map-controls" id="mapControls" role="region" aria-label="Map controls">
          ...
          <fieldset class="mc-section">
            ...
            <div class="filter-group" id="mcMapStyle">
              <button class="btn active" data-style="carto">Carto</button>
              <button class="btn" data-style="satellite">Satellite</button>
              <button class="btn" data-style="hillshade">Hillshade</button>
            </div>
            <div id="mcSatelliteOptions" style="display:none;margin-top:6px;">
              <label for="mcSatLabels" style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:12px;">
                <input type="checkbox" id="mcSatLabels"> Road &amp; label overlay
              </label>
            </div>
            <div id="mcHillshadeOptions" style="display:none;margin-top:6px;">
              <label for="mcHillshadeOpacity" style="display:flex;align-items:center;gap:6px;font-size:11px;margin-bottom:4px;cursor:pointer;">
                Opacity: <span id="mcHillshadeOpacityVal">100%</span>
                <input type="range" id="mcHillshadeOpacity" min="10" max="100" step="5" value="100" style="flex:1;min-width:0;">
              </label>
              <div id="mcHillshadeCounties" style="font-size:12px;"></div>
            </div>
          </fieldset>
          <fieldset class="mc-section">
    ...
    // Active state
    let _mapStyle = localStorage.getItem('meshcore-map-style') || 'carto';
    // _demType removed — combined tiles auto-selected
    let _activeTileLayer = null;
    let _labelLayer = null;       // Carto label overlay on satellite
    let _hillshadeLayers = {};   // county -> L.tileLayer (currently visible overlays)
    let _availableServices = []; // [{name, url}] from tile server
    let _hillshadeOpacity = parseFloat(localStorage.getItem('meshcore-map-hillshade-opacity') || '1.0');

    // Tile layer factories for base styles
    const _tileDefs = {
      cartoDark:  () => L.tileLayer(TILE_DARK,  { attribution: '&copy; OpenStreetMap &copy; CartoDB', maxZoom: 19 }),
      cartoLight: () => L.tileLayer(TILE_LIGHT, { attribution: '&copy; OpenStreetMap &copy; CartoDB', maxZoom: 19 }),
      satellite:  () => L.tileLayer(
        'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
        { attribution: '&copy; Esri &copy; Maxar', maxZoom: 19 }
      ),
      cartoLabels: () => L.tileLayer(
        'https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}{r}.png',
        { attribution: '&copy; CartoDB', maxZoom: 19, subdomains: 'abcd' }
      ),
    };

    // Parse county name from service name.
    // Preferred: "{county}-hillshade-combined-{dark|light}-9x" (3DEP+LiDAR)
    // Fallback:  "{county}-hillshade-{dark|light}-{dsm|dtm}" (legacy)
    // Also:      "{county}-lidar-{dark|light}-9x" (standalone LiDAR)
    function _parseHillshadeService(name) {
      // Combined 3DEP+LiDAR (highest quality)
      let m = name.match(/^([a-z]+)-hillshade-combined-(dark|light)-9x$/);
      if (m) return { county: m[1], style: m[2], source: 'combined' };
      // Standalone LiDAR
      m = name.match(/^([a-z]+)-lidar-(dark|light)-9x$/);
      if (m) return { county: m[1], style: m[2], source: 'lidar' };
      // Legacy DSM/DTM (lowest priority)
      m = name.match(/^([a-z]+)-hillshade-(dark|light)-(dsm|dtm)$/);
      if (m) return { county: m[1], style: m[2], source: 'legacy' };
      return null;
    }

    function _countyLabel(county) {
      return county.charAt(0).toUpperCase() + county.slice(1) + ' Co.';
    }

    // Build hillshade overlay layers for current theme, add to map.
    // Picks the best available source per county: combined > lidar > legacy.
    function _applyHillshadeLayers(services, theme) {
      // Remove existing hillshade overlays
      Object.values(_hillshadeLayers).forEach(function(l) { map.removeLayer(l); });
      _hillshadeLayers = {};

      const savedCounties = JSON.parse(localStorage.getItem('meshcore-map-hillshade-counties') || 'null');

      // Rank sources: combined > lidar > legacy
      const RANK = { combined: 3, lidar: 2, legacy: 1 };
      const best = {}; // county -> { svc, rank }
      services.forEach(function(svc) {
        const parsed = _parseHillshadeService(svc.name);
        if (!parsed || parsed.style !== theme) return;
        const rank = RANK[parsed.source] || 0;
        if (!best[parsed.county] || rank > best[parsed.county].rank) {
          best[parsed.county] = { svc: svc, rank: rank };
        }
      });

      Object.keys(best).forEach(function(county) {
    ...
    // Rebuild county checkbox UI
    function _buildCountyUI(services, theme) {
      const container = document.getElementById('mcHillshadeCounties');
      if (!container) return;
      container.innerHTML = '';

      // Deduplicate counties (best source already picked in _applyHillshadeLayers)
      const available = [];
      const seen = {};
      services.forEach(function(svc) {
        const parsed = _parseHillshadeService(svc.name);
        if (parsed && parsed.style === theme && !seen[parsed.county]) {
          seen[parsed.county] = true;
          available.push(parsed.county);
        }
      });

      if (available.length === 0) {
        container.innerHTML = '<span style="color:var(--text-muted);font-size:11px;">No hillshade tiles available for this combination yet.</span>';
        return;
      }

      const savedCounties = JSON.parse(localStorage.getItem('meshcore-map-hillshade-counties') || 'null');

      available.forEach(function(county) {
        const enabled = savedCounties ? savedCounties.includes(county) : true;
        const id = 'mcHillshade_' + county;
        ...
        cb.id = id;
        cb.checked = enabled;
        cb.dataset.county = county;
        const span = document.createElement('span');
        span.textContent = _countyLabel(county);
        const badge = document.createElement('span');
        badge.style.cssText = 'margin-left:auto;font-size:10px;color:#22c55e;font-weight:600;white-space:nowrap;';
        badge.textContent = '✓';
        badge.title = 'Tiles available';
        ...
        cb.addEventListener('change', function() {
          const layer = _hillshadeLayers[county];
          if (!layer) return;
          if (cb.checked) { layer.addTo(map); } else { map.removeLayer(layer); }
          // Persist
          const all = Object.keys(_hillshadeLayers);
          const on = all.filter(function(c) {
            const l = document.getElementById('mcHillshade_' + c);
            return l && l.checked;
          });
          localStorage.setItem('meshcore-map-hillshade-counties', JSON.stringify(on));
        });
      });
    }

    // Fetch services once, cache in memory
    let _fetchServicesPromise = null;
    async function _fetchServices() {
      if (_availableServices.length > 0) return _availableServices;
      if (_fetchServicesPromise) return _fetchServicesPromise;
      const container = document.getElementById('mcHillshadeCounties');
      if (container) container.innerHTML = '<span style="color:var(--text-muted);font-size:11px;">⏳ Loading tile services…</span>';
      _fetchServicesPromise = (async () => {
        try {
          const resp = await fetch(TILE_SERVER + '/services');
          _availableServices = await resp.json();
        } catch(e) {
          console.warn('[map-style] Could not fetch tile services:', e);
          _availableServices = [];
        }
        _fetchServicesPromise = null;
        return _availableServices;
      })();
      return _fetchServicesPromise;
    }

    function _setBaseLayer(style, theme) {
      if (_activeTileLayer) { map.removeLayer(_activeTileLayer); _activeTileLayer = null; }
      switch(style) {
        case 'satellite':
          _activeTileLayer = _tileDefs.satellite();
          break;
        case 'hillshade':
          // Hillshade: Carto basemap (theme-aware), hillshade tiles overlay on top
          _activeTileLayer = theme === 'dark' ? _tileDefs.cartoDark() : _tileDefs.cartoLight();
          break;
        default:
          _activeTileLayer = theme === 'dark' ? _tileDefs.cartoDark() : _tileDefs.cartoLight();
      }
    ...
    async function _setMapStyle(style, _unused, theme) {
      theme = theme || getTheme();

      // Remove existing hillshade overlays when leaving hillshade mode
      if (style !== 'hillshade') {
        Object.values(_hillshadeLayers).forEach(function(l) { map.removeLayer(l); });
        _hillshadeLayers = {};
      }

      // Remove label overlay when leaving satellite mode
      if (style !== 'satellite' && _labelLayer) {
        map.removeLayer(_labelLayer);
        _labelLayer = null;
      }

      _setBaseLayer(style, theme);

      if (style === 'hillshade') {
        const services = await _fetchServices();
        _applyHillshadeLayers(services, theme);
        _buildCountyUI(services, theme);
      }

      // Re-apply satellite label overlay if checkbox is checked
      if (style === 'satellite') {
        const satLabelsEl = document.getElementById('mcSatLabels');
        if (satLabelsEl && satLabelsEl.checked) {
          _labelLayer = _tileDefs.cartoLabels();
          _labelLayer.addTo(map);
        }
      }

      // Persist
      localStorage.setItem('meshcore-map-style', style);
      // demType no longer used — combined tiles selected automatically

      // Update style button states
      document.querySelectorAll('#mcMapStyle .btn').forEach(function(b) {
        b.classList.toggle('active', b.dataset.style === style);
      });
      // Show/hide style-specific options
      const hillshadeOpts = document.getElementById('mcHillshadeOptions');
      if (hillshadeOpts) hillshadeOpts.style.display = style === 'hillshade' ? '' : 'none';
      const satOpts = document.getElementById('mcSatelliteOptions');
      if (satOpts) satOpts.style.display = style === 'satellite' ? '' : 'none';
    }

    // Init — restore satellite labels checkbox state before applying map style
    (function() {
      const satLabelsEl = document.getElementById('mcSatLabels');
      if (satLabelsEl && localStorage.getItem('meshcore-map-sat-labels') === 'true') {
        satLabelsEl.checked = true;
      }
    })();
    _setMapStyle(_mapStyle, null, getTheme());

    // Map Style buttons
    document.querySelectorAll('#mcMapStyle .btn').forEach(function(btn) {
      btn.addEventListener('click', function() {
        _mapStyle = btn.dataset.style;
        _setMapStyle(_mapStyle, null, getTheme());
      });
    });

    // Satellite labels overlay checkbox
    (function() {
      const satLabelsEl = document.getElementById('mcSatLabels');
      if (!satLabelsEl) return;
      const saved = localStorage.getItem('meshcore-map-sat-labels');
      if (saved === 'true') satLabelsEl.checked = true;
      satLabelsEl.addEventListener('change', function() {
        localStorage.setItem('meshcore-map-sat-labels', satLabelsEl.checked);
        if (satLabelsEl.checked) {
          if (!_labelLayer) { _labelLayer = _tileDefs.cartoLabels(); _labelLayer.addTo(map); }
        } else {
    ...
    (function() {
      const sliderEl = document.getElementById('mcHillshadeOpacity');
      const valEl = document.getElementById('mcHillshadeOpacityVal');
      if (!sliderEl || !valEl) return;
      const pct = Math.round(_hillshadeOpacity * 100);
      sliderEl.value = pct;
      valEl.textContent = pct + '%';
      sliderEl.addEventListener('input', function() {
        const v = parseInt(sliderEl.value, 10);
        valEl.textContent = v + '%';
        _hillshadeOpacity = v / 100;
        localStorage.setItem('meshcore-map-hillshade-opacity', _hillshadeOpacity);
        Object.values(_hillshadeLayers).forEach(function(l) { l.setOpacity(_hillshadeOpacity); });
      });
    })();

    // (DSM/DEM toggle removed — combined tiles auto-selected)

    // Theme observer — Carto and Hillshade both follow dark/light
    const _mapThemeObs = new MutationObserver(function () {
      const theme = getTheme();
      if (_mapStyle === 'carto' || _mapStyle === 'hillshade') {
        _setMapStyle(_mapStyle, null, theme);
      }
    });

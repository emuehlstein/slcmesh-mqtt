...
(function () {
  ...
  async function init(container) {
          ...
          <fieldset class="mc-section">
            ...
            <div id="mcHillshadeOptions" style="display:none;margin-top:6px;">
              <div class="filter-group" style="margin-bottom:6px;">
                <button class="btn active" data-dem="dsm">DSM</button>
                <button class="btn" data-dem="dtm">DEM</button>
              </div>
              <div style="font-size:10px;color:var(--text-muted);margin-bottom:5px;">
                DSM = surface &nbsp;|&nbsp; DEM = bare ground
              </div>
    ...
    // shows per-county checkboxes, supports DSM/DEM sub-toggle.
    ...
    let _demType  = localStorage.getItem('meshcore-map-dem-type') || 'dsm';
    ...
    // Parse county name from service name: "{county}-hillshade-{style}-{demtype}"
    function _parseHillshadeService(name) {
      const m = name.match(/^([a-z]+)-hillshade-(dark|light)-(dsm|dtm)$/);
      if (!m) return null;
      return { county: m[1], style: m[2], demType: m[3] };
    }
    ...
    // Build hillshade overlay layers for current theme + demType, add to map
    function _applyHillshadeLayers(services, theme, demType) {
      ...
      services.forEach(function(svc) {
        ...
        if (!parsed) return;
        if (parsed.style !== theme || parsed.demType !== demType) return;

        const county = parsed.county;
        // Default: all available counties enabled
        const enabled = savedCounties ? savedCounties.includes(county) : true;

        const layer = L.tileLayer(
          TILE_SERVER + '/services/' + svc.name + '/tiles/{z}/{x}/{y}.png',
    ...
    function _buildCountyUI(services, theme, demType) {
      ...
      services.forEach(function(svc) {
        const parsed = _parseHillshadeService(svc.name);
        if (parsed && parsed.style === theme && parsed.demType === demType) {
    ...
    async function _setMapStyle(style, demType, theme) {
      theme = theme || getTheme();
      demType = demType || _demType;
      ...
      if (style === 'hillshade') {
        ...
        _applyHillshadeLayers(services, theme, demType);
        _buildCountyUI(services, theme, demType);
      }
      ...
      localStorage.setItem('meshcore-map-dem-type', demType);
    ...
    _setMapStyle(_mapStyle, _demType, getTheme());
    ...
    // DSM/DEM sub-toggle
    document.querySelectorAll('#mcHillshadeOptions [data-dem]').forEach(function(btn) {
      btn.classList.toggle('active', btn.dataset.dem === _demType);
      btn.addEventListener('click', function() {
        _demType = btn.dataset.dem;
        document.querySelectorAll('#mcHillshadeOptions [data-dem]').forEach(function(b) {
          b.classList.toggle('active', b.dataset.dem === _demType);
        });
        if (_mapStyle === 'hillshade') _setMapStyle(_mapStyle, _demType, getTheme());
      });
    });
    ...
    const _mapThemeObs = new MutationObserver(function () {
      ...
      if (_mapStyle === 'carto' || _mapStyle === 'hillshade') {
        _setMapStyle(_mapStyle, _demType, theme);
      }
    });
